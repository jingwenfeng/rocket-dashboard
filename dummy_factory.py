...
class DummyRocket:
    ...
    def __init__(self):
        self.time_series_data = {"Time": [], "Altitude": [], "Velocity": [], "Acceleration":[], "Temperature":[]}
        self.gravity = 9.8
        self.burnout_time = 5
        self.apogee_time = 10
    ...
    def update_state(self, elapsed_time):
        if elapsed_time <= self.burnout_time:
            ...
            self.acceleration = 0.7 * self.gravity * elapsed_time
            self.temperature = 0.2 * self.gravity * elapsed_time ** 3
        else:
            ...
            self.acceleration = (
                    0.3 * self.gravity * self.burnout_time ** 2
                    - 0.5 * 9.8 * time_since_burnout ** 2
            )
            self.temperature = (
                    0.2 * self.gravity * self.burnout_time ** 2
                    - 0.5 * 9.8 * time_since_burnout ** 2
            )
